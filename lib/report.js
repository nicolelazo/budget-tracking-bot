  ...
  function buildWeeklyReport(state, now) {
    ...
    const recentItems = recent
      .map(h => ({
        ...
        type: h.type === 'CA Request' ? 'Cash Advance' : 'Liquidation',
  ...
  function formatTelegram(r) {
    ...
    if (!items.length) {
      ...
      items.slice(0, MAX_ITEMS).forEach(it => {
        // Cash advances usually carry no amount yet — the figure is only known
        // once the chapter submits its liquidation, so show the amount for
        // liquidations only.
        L.push(it.type === 'Liquidation'
          ? `• <b>${tesc(it.chapter)}</b> — ${tesc(php(it.amount))} <i>(Liquidation)</i>`
          : `• <b>${tesc(it.chapter)}</b> <i>(Cash Advance)</i>`);
    ...
    L.push('');
    // Which chapters filed a cash advance this week (names, not amounts — the
    // amount is only known once the chapter liquidates).
    const caChapters = [];
    (r.weekly.items || []).forEach(it => {
      if (it.type === 'Cash Advance' && caChapters.indexOf(it.chapter) === -1) caChapters.push(it.chapter);
    });
    L.push('<b>Summary of the week:</b>');
    L.push(`• New Liquidations: ${tesc(php(r.weekly.newLiquidated))}`);
    L.push(`• New Cash Advances: ${caChapters.length ? tesc(caChapters.join(', ')) : 'None this week'}`);
    L.push(`• Open Audit Flags: ${r.weekly.openFlags}`);

    if (r.overBudget.length) {
      L.push('');
      L.push('<b>🚨 II. Chapters Over Budget</b>');
