...
(function (global, factory) {
  ...
  function formatTelegram(r) {
    ...
    // ---- I. Seed Fund advanced vs liquidated ----
    L.push('<b>💰 I. Seed Fund: Advanced vs Liquidated</b>');
    L.push(`• Seed Fund Advanced: <b>${tesc(php(t.seedAdvanced))}</b> (${t.seedCount} request${t.seedCount === 1 ? '' : 's'})`);
    L.push(`• Liquidated (confirmed): <b>${tesc(php(t.liquidated))}</b> (${t.liqCount} entr${t.liqCount === 1 ? 'y' : 'ies'})`);
    L.push(`• Outstanding: <b>${tesc(php(t.outstanding))}</b>`);
    L.push('');

    // ---- II. This week's activity ----
    L.push('<b>📅 II. Activity This Week (last 7 days)</b>');
    ...
    // ---- III. Seed funds pending liquidation + due dates ----
    L.push('');
    L.push('<b>🕓 III. Seed Funds Pending Liquidation &amp; Due Dates</b>');
    const pend = (r.pending || []);
    if (!pend.length) {
      L.push('✅ No pending liquidations — every seed fund is accounted for.');
    } else {
