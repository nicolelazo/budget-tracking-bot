...
(function (global, factory) {
  ...
  // liquidated once a human explicitly confirms it (h.liquidated === true) —
  // everything else (new entries, and legacy entries with no flag) is treated as
  // still awaiting liquidation until someone marks it in the dashboard.
  ...
  function buildWeeklyReport(state, now) {
    ...
    // a Cash Advance request or a Liquidation, and liquidations additionally carry
    ...
    const recentItems = recent
      .map(h => ({
        ...
        label: labelOf(h),
        amount: Number(h.amount) || 0,
        kind: isLiqType(h) ? 'liquidation' : 'ca',
    ...
    const weekly = {
      ...
      newCA: recent.filter(h => !isLiqType(h)).reduce((s, h) => s + (Number(h.amount) || 0), 0),
      caCount: recent.filter(h => !isLiqType(h)).length,
  ...
  function formatTelegram(r) {
    ...
    if (!items.length) {
      ...
      items.slice(0, MAX_ITEMS).forEach(it => {
        // Every item is either a Cash Advance request or a Liquidation; a
        // liquidation is only "Liquidated" once a human has confirmed it.
        const tag = it.kind === 'ca'
          ? '💵 <i>(Cash Advance)</i>'
    ...
    L.push(`• Cash Advance Requests: ${tesc(php(r.weekly.newCA))} (${r.weekly.caCount})`);
