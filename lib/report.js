...
(function (global, factory) {
  ...
  function formatTelegram(r) {
    ...
    L.push('');
    L.push('<b>💰 Cash Advanced vs Liquidated</b>');
    L.push(`Advanced: ${tesc(php(r.totals.cashAdvanced))} (${r.totals.caCount})`);
    L.push(`Liquidated: ${tesc(php(r.totals.liquidated))} (${r.totals.liqCount})`);
    L.push(`Outstanding: <b>${tesc(php(r.totals.outstanding))}</b>`);
    L.push('');

    L.push('<b>🧾 Pending CA Liquidations</b>');
    if (!r.pending.length) L.push('None — all cash advances are liquidated. ✅');
    else {
    ...
    L.push('<b>📅 This Week (last 7 days)</b>');
    L.push(`New submissions: ${r.weekly.newSubmissions}`);
    L.push(`Liquidated: ${tesc(php(r.weekly.newLiquidated))} · New CAs: ${tesc(php(r.weekly.newCA))}`);
    L.push(`Open audit flags: ${r.weekly.openFlags}`);
    if (r.overBudget.length) {
      L.push('');
      L.push('<b>🚨 Over Budget</b>');
