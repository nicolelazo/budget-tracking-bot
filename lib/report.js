...
(function (global, factory) {
  ...
  function buildWeeklyReport(state, now) {
    ...
    const recentItems = recent
      .map(h => ({
        ...
        type: h.type === 'CA Request' ? 'Seed Fund Request' : 'Liquidation',
  ...
  function formatTelegram(r) {
    ...
    L.push('');
    // ---- I. Seed funds requested vs. liquidated ----
    L.push('<b>💰 I. Seed Funds Requested vs. Liquidated</b>');
    L.push(`• Total Seed Funds Requested: ${tesc(php(r.totals.cashAdvanced))} (${r.totals.caCount} request${r.totals.caCount === 1 ? '' : 's'})`);
    ...
    // ---- II. Pending seed fund liquidations, grouped by when they are due ----
    L.push('<b>🧾 II. Seed Fund Requests Pending Liquidation</b>');
    if (!r.pending.length) L.push('None. All seed fund requests have been fully liquidated. ✅');
    else {
      L.push('<i>Grouped by liquidation due date:</i>');
    ...
    // ---- III. This week's activity, with the source of each amount ----
    ...
    L.push('');
    // Which chapters filed a seed fund request this week (names, not amounts).
    const sfChapters = [];
    (r.weekly.items || []).forEach(it => {
      if (it.type === 'Seed Fund Request' && sfChapters.indexOf(it.chapter) === -1) sfChapters.push(it.chapter);
    });
    L.push('<b>Summary of the week:</b>');
    L.push(`• New Liquidations: ${tesc(php(r.weekly.newLiquidated))}`);
    L.push(`• New Seed Fund Requests: ${sfChapters.length ? tesc(sfChapters.join(', ')) : 'None this week'}`);
